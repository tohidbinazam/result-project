...
console.log(`
    ...
    Bangla          ${bn}           ${result.gpa(bn)}         ${result.gread(bn)}
    English         ${en}           ${result.gpa(en)}         ${result.gread(en)}
    Math            ${math}         ${result.gpa(math)}       ${result.gread(math)}
    Social Science  ${ss}           ${result.gpa(ss)}         ${result.gread(ss)}
    Science         ${s}            ${result.gpa(s)}          ${result.gread(s)}
    religion        ${rel}          ${result.gpa(rel)}        ${result.gread(rel)}
------------------------------------------------------------------------------------
    Cgpa ${result.finalCgpa( result.gpa(bn), result.gpa(en), result.gpa(math),
        result.gpa(ss), result.gpa(s), result.gpa(rel) )}
